---
meta:
  title: Dr. Yousef Abu Koush | Dental Clinic
  description: Dr. Yousef Abu Koush — Expert dental care you can trust. Modern
    dentistry with a gentle touch.
hero:
  eyebrow: Your Trusted Dental Partner
  title: A Healthier Smile
  title_sub: Starts Here
  lead: State-of-the-art dentistry delivered with compassion and precision. From
    routine check-ups to complete smile makeovers — we've got you covered.
  cta_primary_text: Book an Appointment
  cta_secondary_text: Explore Services
  bg_image: https://images.unsplash.com/photo-1629909613654-28e377c37b09?w=1600&q=80&auto=format&fit=crop
  trust_items:
    - text: 20+ Years Experience
    - text: 3 500+ Happy Patients
    - text: Certified & Insured
services:
  visible: true
  eyebrow: What We Offer
  title: Our Dental Services
  subtitle: From preventive care to complete smile transformations, we provide
    comprehensive dental treatments in a comfortable, modern environment.
  items:
    - title: General Dentistry
      icon: tooth
      description: Routine exams, X-rays, professional cleaning and cavity fillings to
        keep your teeth healthy and strong year after year.
    - title: Teeth Whitening
      icon: smile
      description: Professional-grade in-office whitening that removes years of
        staining in a single visit — safe, fast and dramatically effective.
    - title: Dental Implants
      icon: implant
      description: Permanent, natural-looking replacements for missing teeth. Titanium
        implants that look, feel and function just like your real teeth.
    - title: Orthodontics
      icon: braces
      description: Traditional braces and clear-aligner therapy to straighten teeth
        and correct bite issues for patients of all ages.
    - title: Porcelain Veneers
      icon: veneer
      description: Ultra-thin ceramic shells bonded to the front surface of teeth to
        instantly correct shape, size, colour and minor misalignment.
    - title: Root Canal Therapy
      icon: rootcanal
      description: Gentle, pain-free treatment to save an infected tooth, eliminate
        discomfort and restore full function — often in a single appointment.
about:
  visible: true
  experience_years: 20+
  bio1: Dr. Yousef Abu Koush is a highly experienced general and cosmetic dentist
    with over 20 years of dedicated practice. He earned his dental degree with
    distinction and completed advanced training in implantology and aesthetic
    dentistry at leading European institutions.
  eyebrow: Meet the Doctor
  bio2: "His philosophy is simple: every patient deserves pain-free, honest, and
    personalised care. Whether you are coming in for a routine clean or a full
    smile makeover, Dr. Abu Koush takes the time to listen, explain, and deliver
    results that last."
  name: Dr. Yousef Abu Koush
  photo: /doc.png
  cta_text: Book a Consultation
  photo_alt: Dr. Yousef Abu Koush in his dental clinic
  highlights:
    - Board-Certified General & Cosmetic Dentist
    - Fellow of the International College of Dentists
    - Advanced Training in Dental Implants & Veneers
    - Fluent in Arabic, English & Hebrew
testimonials:
  visible: true
  eyebrow: Patient Stories
  title: What Our Patients Say
  subtitle: Real experiences from real patients. We are proud of the trust our
    community has placed in us.
  items:
    - name: Rania M.
      treatment: Porcelain Veneers Patient
      quote: Dr. Abu Koush completely transformed my smile with veneers. I was nervous
        at first, but he took his time to explain every step. The results are
        beyond what I imagined. I smile with confidence for the first time in
        years!
      avatar: https://i.pravatar.cc/80?img=47
    - name: Khalid A.
      treatment: Root Canal Patient
      quote: I had a root canal done here and I was shocked at how painless it was.
        The clinic is spotless, the staff are incredibly warm, and Dr. Yousef is
        genuinely one of the most skilled and caring dentists I've ever visited.
      avatar: https://i.pravatar.cc/80?img=12
    - name: Lina S.
      treatment: Teeth Whitening Patient
      quote: My whole family has been coming here for over 10 years — my kids, my
        parents, and myself. Dr. Abu Koush remembers every patient by name. The
        whitening treatment gave me results I could see immediately. Absolutely
        recommend!
      avatar: https://i.pravatar.cc/80?img=32
    - name: Omar T.
      treatment: Dental Implant Patient
      quote: Getting implants felt like a big decision, but Dr. Abu Koush walked me
        through every detail. Six months later my implant feels completely
        natural. The investment was absolutely worth it and I wish I had done it
        sooner.
      avatar: https://i.pravatar.cc/80?img=68
stats:
  - number: 3,500+
    label: Patients Treated
  - number: 20+
    label: Years in Practice
  - number: 98%
    label: Satisfaction Rate
  - number: "12"
    label: Services Offered
gallery:
  visible: true
  eyebrow: Our Clinic & Results
  title: A Glimpse Inside
  subtitle: Modern equipment, a calming atmosphere, and smile results that speak
    for themselves.
  items:
    - src: https://images.unsplash.com/photo-1584467735815-f778f274e296?w=800&q=80&auto=format&fit=crop
      alt: Dentist examining a patient in a modern dental chair
      caption: Gentle, attentive care
      tall: true
      wide: false
    - src: https://images.unsplash.com/photo-1606811841689-23dfddce3e95?w=800&q=80&auto=format&fit=crop
      alt: Modern dental clinic reception area
      caption: Welcoming reception
      tall: false
      wide: false
    - src: https://images.unsplash.com/photo-1609840114035-3c981b782dfe?w=800&q=80&auto=format&fit=crop
      alt: State-of-the-art dental equipment
      caption: State-of-the-art equipment
      tall: false
      wide: false
    - src: https://images.unsplash.com/photo-1598256989800-fe5f95da9787?w=1200&q=80&auto=format&fit=crop
      alt: Patient smiling with perfect white teeth after treatment
      caption: Real smile results
      tall: false
      wide: true
    - src: https://images.unsplash.com/photo-1571772996211-2f02c9727629?w=800&q=80&auto=format&fit=crop
      alt: Dentist discussing treatment plan with patient
      caption: Clear communication, every visit
      tall: false
      wide: false
contact:
  visible: true
  eyebrow: Get In Touch
  title: Book Your Appointment
  lead: Ready for a healthier, more confident smile? Fill out the form and our
    team will confirm your appointment within 24 hours.
  address: 14 Al-Nahda Street, Amman, Jordan
  phone: +962 6 555 0192
  email: info@abukoush-dental.com
  hours_weekday: "Sun – Thu: 9:00 AM – 7:00 PM"
  hours_weekend: "Fri – Sat: 10:00 AM – 3:00 PM"
footer:
  tagline: Caring for your smile since 2004. Modern dentistry with a gentle,
    personal touch.
---
